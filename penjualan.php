...
if(isset($_POST['submit'])){
    ...
    if($stok >= 0){
        ...
        if($query){
            mysqli_query($conn, "insert into penjualan values('', '$kode', '$jumlah', '$tanggal')");
            echo "Sukses melakukan penjualan";
        }else{
            echo "gagal melakukan penjualan";
        }
    }else{
        echo "Penjualan dilarang melebihi sisa stok";
    }
}
?>
        ...
        <form action="" method="post">
            ...
            <div class="form-controll">
                ...
                <select name="kode" required>
                    ...
                    foreach($produk as $p){
                    ?>
                    <option value="<?=$p['kode'];?>"><?=$p['kode'];?> - <?=$p['nama'];?> | <?=$p['nama_supplier'];?> | <?=$p['stok'];?> Box</option>
                    <?php    
                    }
                    ?>
                </select>
            ...
            <div class="form-controll">
                ...
                <br>
                <input type="number" name="jumlah" placeholder="Jumlah penjualan" required>
            </div>
